...
function random() payable public costs (10000 wei) {
  ...
  function saveResults(bytes32, uint value) public{
  ...
  function winOrLose() internal returns (uint){
      if (results[latestNumber].isWin == true){
          ...
          uint toReward = waiting[id].betAmount * 2;

          msg.sender.transfer(toReward);
          balance -= waiting[id].betAmount;
          ...
          //keep money
          balance += waiting[id].betAmount;
          emit youLose("You lost ", waiting[id].betAmount);
      }


      waiting[id].betAmount = 0;
  }
